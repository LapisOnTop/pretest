...
/**
 * Populates a given select element with country dial codes.
 * @param {string} selectElementId - The ID of the select element to populate.
 * @param {string} defaultCode - The default country/dial code to select (e.g., 'PH' or '+63').
 */
function populateCountryDropdown(selectElementId, defaultCode = 'PH') {
    ...
    countries.forEach(country => {
        ...
        // Setting value to the dial code
        option.value = country.dialCode;
        option.textContent = `${country.code} (${country.dialCode})`;
        // You could also do: option.textContent = `${country.name} (${country.dialCode})`;

        // Let's set the text content to be just the country code to save space exactly as in the mock,
        // but with hover showing full name
        option.textContent = country.code;
        option.title = `${country.name} (${country.dialCode})`;

        // Select matching default (match by standard ISO code or standard dial code)
        if (country.code === defaultCode || country.dialCode === defaultCode) {
